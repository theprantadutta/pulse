...
class PulseMark extends StatelessWidget {
  ...
  String _svg(WireColors w) {
    ...
    if (contained) {
      final bg = colorway == PulseMarkColorway.mono ? '' : '<rect width="104" height="104" fill="${_hex(square)}"/>';
...
class PulseLockup extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final word = Text('PULSE', style: WireType.display(height * 0.94, width: 75, height: 1).copyWith(color: fg));
